...
const Romance = () => {
  ...
  const selected = FetchedMovies &&    FetchedMovies.filter((item) => item.category.tittle === "Romance")
  const totalPages =
    selected && Math.ceil(selected.length / itemsPerPage);

  // Paginated movies for the current page
  const paginatedMovies = useMemo(() => {
    return (selected || [])
      .slice((page - 1) * itemsPerPage, page * itemsPerPage)
      .filter((item) => item.category.tittle === "Romance");
  }, [FetchedMovies, page]);
    ...
    <Layout>
      <div>
        <div className="flex w-full sm:gap-5 gap-4 items-center my-14">
          ...
          <h2 className="sm-text-xl text-lg font-bold">Romance Movies</h2>
        </div>
