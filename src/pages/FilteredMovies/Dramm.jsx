...
const Dramma = () => {
  ...
  const selected =
    FetchedMovies &&
    FetchedMovies.filter((item) => item.category.tittle === "Dramma");
  ...
  const paginatedMovies = useMemo(() => {
    return (selected || [])
      ...
      .filter((item) => item.category.tittle === "Dramma");
  }, [FetchedMovies, page]);
    ...
    <Layout>
      <div>
        <div className="flex w-full sm:gap-5 gap-4 items-center my-14">
          ...
          <h2 className="sm-text-xl text-lg font-bold">Dramma Movies</h2>
        </div>
