  ...
  return (
    <div className="bg-dry py-4  px-4 border-2 border-black w-full">
      <div className="flex  flex-wrap gap-4 md:gap-10  items-center w-full justify-between md:justify-center">
        {Links.map((link) => (
          <div className="flex flex-col justify-start  items-start my-2 md:my-0 footText gap-5 md:gap-6 w-[40%] md:w-[20%] ">
            <p className=" font-semibold">{link.title}</p>
            <div className="flex flex-col gap-4 justify-start items-start">
              {link.links.map((item) => (
                <p className="text-border transi hover:text-subMain cursor-pointer  ">
                  {item.title}
                </p>
              ))}
            </div>
